...
//	var err error
//	mDb[DbName], err = Connect(uri, max, min)
//	if err != nil {
//		println(err.Error())
//	}
//}
//
////Connect db by dsn e.g. "user:password@tcp(127.0.0.1:3306)/dbname?parseTime=true&loc=Local"
//func Connect(dsn string, max, min int) (*sql.DB, error) {
...
//func Table(DbName, tableName string) func(...Dba) *Query {
...
//	}
//	query := fmt.Sprintf(`select %s from %s %s %s %s %s`, strings.Join(q.only, ","), q.table, where, q.order, q.limit, q.offset)
//	log.Printf("sql: %s", query)
//	return query
//}
//
...
//		where = fmt.Sprintf(`where %s`, strings.Join(q.wheres, " and "))
//	}
//	query := fmt.Sprintf(`delete from %s %s`, q.table, where)
//	log.Printf("sql: %s", query)
//	st, err := q.DB.Prepare(query)
//	if err != nil {
//		return 0, err
...
//}
//
//func Execute(db *sql.DB, sqlStr string, args ...interface{}) (sql.Result, error) {
